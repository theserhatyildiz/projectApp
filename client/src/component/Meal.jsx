...
export default function Meal({ items, mealNumber, eatenDate, deleteFood }) {
    ...
    const handleFoodClick = (foodItem, details, quantity, id, mealNumber, eatenDate) => {
        navigate("/search", { state: { foodItem, quantity, details, id, mealNumber, eatenDate } });
    };
    ...
    return (
        <div className="meal-container">
                ...
                <div>
                    {filteredItems.map((item) => (
                        <div className="item" key={item._id}>
                            <h3 className="food-heading" onClick={() => handleFoodClick(item.foodId, item.details, item.quantity, item._id, item.mealNumber, item.eatenDate)}>{item.foodId.NameTr}</h3>
                            <div className="food-info-container">
                                <div className="food-info" onClick={() => handleFoodClick(item.foodId, item.details, item.quantity, item._id, item.mealNumber, item.eatenDate)}>
            ...
            <div className="btn-group">
                ...
                <Link to={`/mealFunctions?mealNumber=${mealNumber}&eatenDate=${eatenDate}`}>
